...
def draw(args: argparse.ArgumentParser, summary: Dict):
    fig, axes = plt.subplots(2, 3, figsize=(22, 12), frameon=False)
    ...
    for ax_idx, ax in enumerate(axes.flat):
        ...
        ax.set_xticklabels(
            [model.rstrip("-MLC") for model in models],
            rotation=12,
            ha="center",
            fontsize=9,
        )
    ...
    fig.legend(
        handles=legend_handles,
        labels=["with structual tag", "without structual tag"],
        loc="lower center",
        bbox_to_anchor=(0.5, 0.05),
        ncol=2,
        fontsize=12,
    ...
    plt.tight_layout(rect=[0, 0.1, 1, 0.95])
    plt.savefig(
        f"{args.summary_root}/correct_schema_rate.png", dpi=300, bbox_inches="tight"
    )
