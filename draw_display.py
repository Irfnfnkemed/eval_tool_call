...
def draw(args: argparse.ArgumentParser, summary: Dict):
    ...
    for ax_idx, ax in enumerate(axes.flat):
        ...
        for model in summary:
            draw_info[model] = summary[model][dataset]
        for i, bar in enumerate(bars):
            values = [draw_info[model]["correct_schema_rate"][bar] * 100 for model in models]
            ax.bar(x + i * (width + gap), values, width, color=colors[i], label=bar)
    ...
    plt.subplots_adjust(bottom=0.2, top=0.85, left=0.05, right=0.95, wspace=5, hspace=0.3)  # Increased wspace
    fig.legend(
        handles=legend_handles,
        labels=["without structual tag", "with structual tag"],
        loc="lower center",
